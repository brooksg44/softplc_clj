...
(defn- draw-plc-stats
  ...
  (let [stats (runtime/get-stats)
    ...
    (q/text (str "Last Scan: " (format "%.2f" last-scan-time) " ms") x (+ y 40))
    (q/text (str "Avg Scan: " (format "%.2f" avg-scan-time) " ms") x (+ y 60))
    (q/text (str "Min Scan: " (format "%.2f" min-scan-time) " ms") x (+ y 80))
    (q/text (str "Max Scan: " (format "%.2f" max-scan-time) " ms") x (+ y 100))))
